          ...
          {[0, 1, 2, 3].map((i) => (
            <div
              ...
              className={cn(
                ...
                i === step ? 'w-8 bg-[var(--primary)]' : i < step ? 'w-2 bg-[var(--primary)]' : 'w-2 bg-[var(--border)]'
              )}
            />
          ))}
        </div>
            ...
            <CardContent className="p-8 text-center">
              <div className="flex justify-center mb-6">
                <div className="w-20 h-20 rounded-full bg-[var(--primary)]/10 flex items-center justify-center">
                  <Storefront size={48} className="text-[var(--primary)]" weight="duotone" />
                </div>
              </div>
              <h1 className="text-2xl font-bold mb-2">Selamat datang di LaundryPOS</h1>
              <p className="text-[var(--muted-foreground)] mb-2">{tenantName}</p>
                  ...
                  {outlets.map((outlet) => (
                    <button
                      ...
                      className={cn(
                        ...
                        'hover:border-[var(--primary)]/50 active:scale-[0.99]',
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        {selectedOutletId === outlet.id && (
                          <CheckCircle size={24} className="text-[var(--primary)] shrink-0" weight="fill" />
                        )}
                      </div>
                    </button>
                  ))}
        ...
        {step === 3 && (
          <Card>
            <CardContent className="p-6">
              ...
              <div className="rounded-xl bg-[var(--muted)] p-4 space-y-3 mb-6">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-[var(--muted-foreground)]">Outlet</span>
                  <span className="text-sm font-semibold">{selectedOutletName}</span>
                </div>
                <div className="h-px bg-[var(--border)]" />
                <div className="flex justify-between items-center">
                  <span className="text-sm text-[var(--muted-foreground)]">Nama Perangkat</span>
                  <span className="text-sm font-semibold">{deviceName}</span>
                </div>
                <div className="h-px bg-[var(--border)]" />
                <div className="flex justify-between items-center">
                  <span className="text-sm text-[var(--muted-foreground)]">Pengguna</span>
